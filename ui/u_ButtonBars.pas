...
uses System.Classes, Vcl.Controls, Vcl.Graphics, System.SysUtils;

type
  TCaptionArray = array of string;

  TButtonBar = class(TCustomControl)
  private
    ...
    fItemIndex: Integer;
    fItemIndexBeforeMouseDown: Integer;
    fMouseDown: Boolean;
...
uses System.Math, Vcl.Themes, Vcl.GraphUtil;
...
begin
  ...
  fMouseDown := True;
  fItemIndexBeforeMouseDown := fItemIndex;
  fMouseDownIndex := ItemAt(X, Y);
  ItemIndex := fMouseDownIndex;
  MouseCapture := True;
end;
...
procedure TButtonBar.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  index: Integer;
begin
  inherited;
  if (Button <> mbLeft) or (not fMouseDown) then
    Exit;
  ...
  begin
    ItemIndex := fItemIndexBeforeMouseDown;
  end;
end;
